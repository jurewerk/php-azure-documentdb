...
class AzureRequest {
	...
	public function request($path, $method = 'GET', $options = array(), $resource_type = null, $resource_id = '', $additional_headers = array(), $partition = -1)
	{
		...
		$this->setRequestData($curl, $method, $resource_type, $resource_id, $options, $additional_headers, $partition);
		($this->debug) ?
	...
	protected function setRequestData(&$curl, $method, $resource_type, $resource_id, $options, $additional_headers, $partition)
	{
		...
		{                                        
			...
			if (isset($options['query']))
			{
               ...
               // $my = array(1,2);
               // $my = json_encode($my);
                //$headers[] = 'x-ms-documentdb-partitionkeyrangeid: "0"';
                //$headers[] = 'x-ms-documentdb-partitionkey: ["V-46777"]';
		...
		if($partition > -1) {
            $headers[] = 'x-ms-documentdb-partitionkeyrangeid: ' . $partition;
        }
